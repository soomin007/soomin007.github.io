...
assets/shots/     상세 갤러리 스크린샷 (<게임id>-*.webp, 게임당 2~3장)
...
2. `games.js` 의 `GAMES` 배열에 항목 추가. 나머지 파일은 손대지 않는다.
   ...
   - `shots`: 상세 갤러리 2~3장 (선택). 테마가 액자 연출을 정한다
     (enigma=증거 사진, hud=CAM 피드, eco=표본 도판, 나머지=기본 액자).
3. (선택) 맛보기를 넣으려면 `teasers.js` 에 게임 id 로 등록한다. 형식은 파일 상단 주석 참고.
   타이머·raf 를 쓰면 반드시 cleanup 함수를 반환할 것 (페이지를 떠날 때 호출된다).
