...
## 11. 게임 소개 페이지가 게임의 옛 설계를 실은 채 방치된다 (2026-08-29)

- 증상: 적자생존 페이지가 "조작 없음 · 관전" 이라고 쓰여 있는데 본편은 8월 2일에 탭 지휘로
  바뀌었고, 스크린샷 3장은 이미 사라진 UI(한 마리 관찰 · 개체 카드 · 옛 형질 카드)를 담고 있었다.
  4주 동안 아무도 눈치채지 못했다.
- 규칙: 게임 저장소에서 조작 방식 · 핵심 루프 · 카드 체계가 바뀌면 허브 페이지도 같은 날 갱신한다.
  카드 수 · 시대 수 · 명령 종류 같은 수치는 그 저장소의 코드(cards.ts · perks.ts · config.ts ·
  herdOrder.ts)에서 다시 센다. 기획서·핸드오프 문서는 옛 설계를 담고 있을 수 있다.
- 규칙: 스크린샷은 **라이브 URL 을 playwright 로 찍는다** (게임 저장소의 `node_modules/playwright`
  를 `NODE_PATH` 로 빌려 쓴다 · 모바일 440x940 @2x → 560x1196 webp · 데스크톱 1280x720 @1.5 →
  캡슐 1232x706 · OG 1200x630 · `?seed=` 고정). 로컬 dev 서버도, 크롬 창 전경화(#3)도 필요 없다.
  플레이 흐름이 바뀌어 버튼 이름이 안 맞으면 먼저 보이는 버튼 목록을 덤프해 본다.
